...
def build_evidence_bundle(state: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    claim_history = (
        get_claim_history(policy_no, exclude_claim_id=claim_id) if policy_no else []
    )
...
async def evidence_node(state: Any) -> Dict[str, Any]:
    evidence_bundle = build_evidence_bundle(state)

    workflow_history = list(state.get("workflow_history", []))
    workflow_history.append(
        "EvidenceAgent: loaded policy, claimant, claim history, and hospital reference data"
    )
