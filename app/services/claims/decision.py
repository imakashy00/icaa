...
def _passed_checks(state: Mapping[str, Any]) -> Dict[str, bool]:
    return {
...
def _failed_checks(state: Mapping[str, Any]) -> Dict[str, List[str]]:
    failed: List[str] = []
...
def build_final_report(state: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if failed_reasons:
        final_decision = (
            "Rejected"
            if fraud_score >= 50 or not state.get("coverage_eligible", False)
...
async def decision_node(state: Any) -> Dict[str, Any]:
    final_report = build_final_report(state)
    workflow_history = list(state.get("workflow_history", []))
    workflow_history.append(
        f"DecisionAgent: final decision set to {final_report['decision']}"
    )
