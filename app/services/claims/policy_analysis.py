...
def _sum_claim_expenses(claim_expenses: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if declared_total is not None and abs(computed_total - declared_total) > 1.0:
        validation_warning = f"Declared total_expenses ({declared_total:.2f}) does not reconcile with itemized expenses ({computed_total:.2f})."
...
def check_diagnosis_coverage(
    icd_code: Optional[str], policy: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    if not policy:
    ...
    return {
        ...
        "reason": None
        if code_covered
        else "Diagnosis is not listed as covered under the policy rules.",
...
def calculate_approved_amount(
    ...
    if not policy or not coverage_eligible:
        return {
            ...
            "approved_amount": 0.0,
            "deductions": [],
            "validation_warnings": [amounts["validation_warning"]]
            if amounts["validation_warning"]
            else [],
    ...
    deductions = []
    validation_warnings = (
        [amounts["validation_warning"]] if amounts["validation_warning"] else []
    )
    ...
    if fraud_deduction_pct > 0:
        ...
        deductions.append(
            {"reason": "Fraud risk deduction applied", "amount": fraud_amount}
        )
...
async def policy_analysis_node(state: Any) -> Dict[str, Any]:
    ...
    coverage_result = check_diagnosis_coverage(
        diagnosis.get("primary_icd_code")
        or state.get("primary_icd_code")
        or state.get("diagnosis_code"),
        policy,
    )
    ...
    approval_result = calculate_approved_amount(
        ...
        policy=policy,
        coverage_eligible=coverage_result["coverage_eligible"]
        and waiting_period_completed,
        fraud_deduction_pct=fraud_deduction_pct,
    )
    if approval_result.get("validation_warnings"):
        exclusions_found.extend(
            [warning for warning in approval_result["validation_warnings"] if warning]
        )

    workflow_history = list(state.get("workflow_history", []))
    workflow_history.append(
        "PolicyAgent: evaluated diagnosis coverage, waiting period, exclusions, and approved amount"
    )

    return {
        "policy_active": bool(policy and policy.get("active")),
        "coverage_eligible": bool(
            coverage_result["coverage_eligible"] and waiting_period_completed
        ),
